...
export function NewOrderForm() {
	...
	// Carregar dados iniciais (clientes recentes e produtos populares)
	useEffect(() => {
	...
	// Preencher campos de endereço quando usuário for selecionado
	useEffect(() => {
		if (selectedUser) {
			setAddressLine1(selectedUser.addressLine1 || "");
			setAddressLine2(selectedUser.addressLine2 || "");
	...
	const handleAddProduct = (productId: string) => {
		// Buscar em ambas as listas (produtos da busca ou populares)
		const allProducts = productSearch.length >= 2 ? products : popularProducts;
			...
			prev.map((item) => {
				if (item.id === productId) {
					...
					if (newQuantity > item.stockQuantity) {
						toast.error(
							`Estoque insuficiente. Disponível: ${item.stockQuantity}`,
						);
						return item;
					}
	...
	const setQuantity = (productId: string, quantity: number) => {
		...
		if (item && quantity > item.stockQuantity) {
			toast.error(`Estoque insuficiente. Disponível: ${item.stockQuantity}`);
			return;
		}
					...
					<CardBody className="p-6">
						<Autocomplete
							...
							onSelectionChange={(key) => {
								const allUsers = userSearch.length >= 2 ? users : recentUsers;
								const user = allUsers.find((u) => u.id === key);
								setSelectedUser(user || null);
							}}
							items={userSearch.length >= 2 ? users : recentUsers}
							classNames={{
								listboxWrapper: "max-h-[300px]",
							}}
							listboxProps={{
								emptyContent:
									hasSearched && userSearch.length >= 2 ? (
						...
						{!selectedUser && (
							<div className="mt-4 flex items-center gap-3 rounded-xl border border-dashed border-slate-300 bg-slate-50/50 p-4 dark:border-slate-600 dark:bg-slate-800/30">
								...
								<Button
									size="sm"
									className="bg-brand-600 font-medium text-white shadow-md shadow-brand-500/20 hover:bg-brand-700"
									onPress={handleCreateQuickUser}
									isLoading={creatingUser}
				...
				>
					...
					<CardBody className="p-6">
						<Autocomplete
							...
							items={productSearch.length >= 2 ? products : popularProducts}
							classNames={{
								listboxWrapper: "max-h-[300px]",
							}}
							listboxProps={{
								...
								<AutocompleteItem key={product.id} textValue={product.name}>
									<div className="flex justify-between items-center py-1">
										<div className="flex flex-col flex-1">
											...
											<span className="text-xs text-default-400">
												Cód: {product.code} • Estoque: {product.stockQuantity}
											</span>
										</div>
									...
									>
										...
										<div className="flex items-center gap-1 rounded-lg bg-slate-100 p-1 dark:bg-slate-700/50">
											...
											<Input
												...
												onValueChange={(val) =>
													setQuantity(item.id, parseInt(val, 10) || 1)
												}
												classNames={{
													base: "w-12",
			...
			<div className="space-y-6">
				<Card className="sticky top-6 border border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
					...
					<CardBody className="p-6 space-y-5">
						...
						{/* Total */}
						<div className="rounded-xl bg-brand-500 p-4 text-white shadow-lg shadow-brand-500/25">
							<div className="flex justify-between items-center">
								<div>
									<p className="text-xs text-white/80">Total do Pedido</p>
									<p className="text-2xl font-bold">
										{formatCurrency(calculateTotal())}
									</p>
								</div>
								<CheckBadgeIcon className="h-10 w-10 text-white/50" />
							</div>
						</div>
